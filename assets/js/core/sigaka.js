$(document).ready(function () {
	...
	// start
	// ------------------------------------------------------------------------------------------
	var root = window.location.origin + '/gaji_guru/';
		...
		$.ajax({
			...
			success: function (response) {
				if (response != null){
					...
					$('#lihat_mapel').val(response.karyawan_mapel);
		...
		$.ajax({
			...
			success: function (response) {
				if (response != null){
					...
					$('#edit_mapel').val(response.karyawan_mapel);
...
// ------------------------------------------------------------------------------------------
	...
	$('.absen-lihat').click(function (e) {
		e.preventDefault();
		var id = $(this).val();
		var getUrl = root + 'absen/lihat/' + id;
		$.ajax({
			url : getUrl,
			type : 'ajax',
			dataType : 'json',
	...
	$('.absen-hapus').click(function () {
		var id = $(this).val();
		var html = '' +
			'<a href="'+root+'absen/hapus/'+id+'" class="btn btn-danger btn-bg-gradient-x-red-pink">Hapus</a>';
		$('#hapusabsen').html(html);
	});
	...
	var options = {
		...
		list: {
			...
			onSelectItemEvent: function() {
				var value = $("#nama_karyawan").getSelectedItemData().absen_id;
		...
		$.ajax({
			...
			success: function (response) {
				if (response != null){
					...
					$('#gaji_lihat_tg').val(response.gaji_tanggal);
					$('#gaji_lihat_bulan').val(response.gaji_bulan_ke);
					$('#gaji_lihat_gaji').val(formatRupiah(response.gaji_total,'Rp. '));
					$('#gaji_lihat_gawali').val(formatRupiah(response.gawali,'Rp. '));
					$('#gaji_lihat_trasi').val(formatRupiah(response.trasi,'Rp. '));
					$('#gaji_lihat_majer').val(formatRupiah(response.majer,'Rp. '));
					$('#gaji_lihat_piket').val(formatRupiah(response.piket,'Rp. '));
					$('#gaji_lihat_prog').val(formatRupiah(response.prog,'Rp. '));
					$('#gaji_lihat_barokah').val(formatRupiah(response.barokah,'Rp. '));
			...
			success: function (response) {
				...
				if (response != null){
					html += '' +
						...
						'</div>';
					html += '' +
						'<h2 style="text-align: center">SMP IT FATWAH </h2>' +
					...
					html += '' +
						...
						'<p>Situbondo, '+bulanArr[parseInt(bulan)-1]+' '+tahun+'</p>' +
						'<p>Kepala Sekolah</p>' +
						'<br>' +
						'<br>' +
						'<br>' +
						'<p><b><u>Rahmawati, S.Pd</u></b></p>' +
						'</div>' +
						'</div>';
					$('#laporan').html(html);
				}
